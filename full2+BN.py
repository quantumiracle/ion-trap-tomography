...
def compute_accuracy(v_xs, v_ys, v_p_ys):
    ...
    p_y_pre = sess.run(p_prediction, feed_dict={xs: v_xs, keep_prob: 1, phase_train.name: True})
    #print(y_pre,v_ys)
...
#p nn
p_W_fc1 = weight_variable([num_points/4*64, 1024])
p_b_fc1 = bias_variable([1024])
p_W_fc2 = weight_variable([1024, num_p])
...
p_h_conv1_bn=batch_norm((conv2d(x_image, p_W_conv1) + p_b_conv1), n_out1, phase_train)
p_h_conv1 = tf.nn.tanh(p_h_conv1_bn) # output size 28x28x32
p_h_pool1 = max_pool_2x2(p_h_conv1)                    # output size 14x14x32

## conv2 layer ##
p_h_conv2_bn=batch_norm((conv2d(p_h_pool1, p_W_conv2) + p_b_conv2), n_out2, phase_train)
p_h_conv2 = tf.nn.tanh(p_h_conv2_bn) # output size 14x14x64
...
p_h_fc1 = tf.nn.tanh(full_batch_norm(tf.matmul(p_h_pool2_flat, p_W_fc1) + p_b_fc1, 1024, phase_train))
    ...
    for i in range(3000):
        ...
        if j <= len(train_ys)/persize_train-1:
            ...
            if j % 3 == 0:    #5times p, 1 times ome+gamma
                sess.run(train_step, feed_dict={xs: i_xs, ys: i_ys, keep_prob: 0.5, phase_train.name: True})
            sess.run(p_train_step, feed_dict={xs: i_xs, p_ys:i_p_ys, keep_prob: 0.5, phase_train.name: True})
            j=j+1
            #print(i_ys)
        if i % 5 == 0:
...
if args.test:
    ...
    y_pre_sample = sess.run(prediction, feed_dict={xs: test_xs[:5], keep_prob: 1, phase_train.name: False})
    p_y_pre_sample = sess.run(p_prediction, feed_dict={xs: test_xs[:5], keep_prob: 1, phase_train.name: False})
